...
pub async fn load_server_tls_config(config: &AppConfig) -> Result<ServerTlsConfig> {
    let cert = tokio::fs::read(&config.llm_gateway_service_cert_path)
        .await
        .context(format!(
            "Failed to read cert: {}",
            config.llm_gateway_service_cert_path
        ))?;

    let key = tokio::fs::read(&config.llm_gateway_service_key_path)
        .await
        .context(format!(
            "Failed to read key: {}",
            config.llm_gateway_service_key_path
        ))?;
...
// İstemci tarafı mTLS konfigürasyonu (Llama servisine bağlanmak için)
pub async fn load_client_tls_config(config: &AppConfig) -> Result<ClientTlsConfig> {
    ...
    let cert = tokio::fs::read(&config.llm_gateway_service_cert_path).await?;
    let key = tokio::fs::read(&config.llm_gateway_service_key_path).await?;
    let identity = Identity::from_pem(cert, key);
